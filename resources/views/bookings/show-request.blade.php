    ...
    <div class="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col gap-3">
            ...
            <div>
                ...
                <p class="soh-subtitle">
                    @if($role === 'teacher')
                        Review request details and choose how to proceed with this booking request.
                    @else
                        View your lesson request status.
                    @endif
                </p>
            </div>
        </div>
            ...
            <section class="soh-card p-6 lg:col-span-2">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        @php $sv = $lessonRequest->status->value; @endphp
                        <p class="mt-1">
                            <span class="rounded-full px-3 py-1 text-xs font-semibold {{ match($sv) { 'pending' => 'bg-yellow-100 text-yellow-800', 'teacher_rescheduled' => 'bg-purple-100 text-purple-800', 'teacher_confirmed' => 'bg-blue-100 text-blue-800', 'student_accepted' => 'bg-green-100 text-green-800', 'student_declined', 'cancelled' => 'bg-gray-100 text-gray-800', default => 'bg-gray-100 text-gray-800' } }}">{{ $lessonRequest->status->label() }}</span>
                        </p>
                    </div>
                </div>
                ...
                @if ($lessonRequest->suggested_date)
                    <div class="mt-4 rounded-2xl border border-purple-200 bg-purple-50 p-4">
                        <p class="text-xs font-semibold tracking-wide text-purple-700 uppercase">Suggested Time (Teacher)</p>
            ...
            <section class="space-y-6">
                ...
                @if($role === 'teacher')
                    <article class="soh-card p-6">
                        ...
                        <form method="POST" action="{{ route('bookings.confirm', $lessonRequest) }}" class="mt-4 space-y-3">
                            @csrf
                            @method('PATCH')
                            <div>
                    ...
                    <article class="soh-card p-6">
                        ...
                        <form method="POST" action="{{ route('bookings.teacher-reschedule', $lessonRequest) }}" class="mt-4 space-y-3">
                            @csrf
                            @method('PATCH')
                            <div>
                    ...
                    <article class="soh-card border border-red-200 p-6">
                        ...
                        <form method="POST" action="{{ route('bookings.reject', $lessonRequest) }}" class="mt-4 space-y-3">
                            @csrf
                            @method('PATCH')
                            <div>
                ...
                {{-- Student: Accept Suggestion --}}
                @if($role === 'student' && $lessonRequest->status === \Modules\Booking\Enums\LessonRequestStatus::TeacherRescheduled)
                    <article class="soh-card p-6">
                        <h2 class="text-lg font-semibold text-black">Accept Suggested Time</h2>
                        <p class="mt-2 text-sm text-gray-600">The teacher suggested a new schedule. Accept it to confirm the lesson.</p>
                        <form method="POST" action="{{ route('bookings.accept-suggestion', $lessonRequest) }}" class="mt-4">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="soh-btn-primary w-full">
