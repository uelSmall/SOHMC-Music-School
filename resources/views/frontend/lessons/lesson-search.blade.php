<div class="mx-auto max-w-7xl space-y-8">
    ...
    <section class="soh-card space-y-5 p-6 shadow-sm">
        <div>
        ...
        <div class="flex gap-6 sm:gap-8">
            @foreach (['all' => 'All Lessons', 'assigned' => 'Assigned', 'completed' => 'Completed'] as $tabKey => $tabLabel)
                <button
                    ...
                    class="border-b-2 px-2 py-3 text-sm font-semibold transition-all duration-200 {{ $tab === $tabKey ? '' : 'border-transparent text-gray-600 hover:text-gray-900' }}"
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3" wire:loading.remove wire:target="search,filterInstrument,filterStatus,tab">
        @forelse ($lessons as $lesson)
            ...
            <article class="soh-card overflow-hidden border-l-4 p-0 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:scale-[1.01] hover:shadow-md" style="border-color:rgba(217,145,205,0.35); border-left-color:{{ $leftBorderColor }};">
                <div class="min-h-[92px] p-5" style="background:rgba(140,3,117,0.9); border-bottom:1px solid #D991CD;">
                    <h3 class="line-clamp-2 text-xl font-semibold leading-snug text-white">{{ $lesson->title }}</h3>
                    @php
                ...
                <div class="space-y-4 p-6">
                    ...
                    @if (auth()->user()->hasRole('student'))
                        <div class="min-h-[126px] space-y-3 border-t border-gray-200 pt-4">
                            @if ($assignment)
                            ...
                            @else
                                <div class="flex items-center justify-between gap-3">
                                    <span class="text-[11px] font-semibold uppercase tracking-wide text-gray-500">Status</span>
                                    <span class="inline-flex items-center rounded-full px-3 py-1.5 text-xs font-semibold ring-1 ring-inset" style="background:#F2F2F2; color:#0D0D0D; border-color:#D991CD;">
                                        Pending
                                    </span>
                                </div>
                                ...
                                <div>
                                    <div class="h-2 w-full overflow-hidden rounded-full" style="background:#F2F2F2;">
                                        <div class="h-full rounded-full" style="width: 0%; background:linear-gradient(90deg,#D991CD 0%,#A6128D 100%);"></div>
                                    </div>
                                    <p class="mt-1 text-[11px] font-medium text-gray-500">Progress: 0%</p>
                                </div>
                    ...
                    <div class="mt-2 flex flex-wrap items-center gap-2 border-t border-gray-200 pt-4">
                        <a
