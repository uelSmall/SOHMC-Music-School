...
@section("content")
    ...
    {{-- ═══════════════════════════════════════════════════════
         HERO
    ═══════════════════════════════════════════════════════ --}}
    <section class="relative overflow-hidden bg-[linear-gradient(160deg,#A6128D_0%,#8C0375_30%,#6B025E_60%,#4A0140_100%)] py-16 sm:py-20 lg:py-24">
        {{-- Subtle radial glow --}}
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
            <div class="flex flex-col items-center text-center">
                ...
                <div class="relative mb-8 sm:mb-10">
                    <div class="absolute -inset-6 rounded-full bg-white/10 blur-2xl"></div>
                    <img
                        src="{{ asset('img/sohmc-nav-logo.png') }}"
                        alt="SOHMC Logo"
                        class="relative h-28 w-auto sm:h-36 lg:h-44 drop-shadow-2xl"
                    />
                </div>

                {{-- Headline --}}
                <h1 class="font-heading text-4xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl">
                    {{ __('Sounds of Harmony') }}<br class="hidden sm:block" />
                    <span class="text-[#D991CD]">{{ __('Music Centre') }}</span>
                </h1>

                {{-- Subheadline --}}
                <p class="mt-5 max-w-2xl text-base leading-relaxed text-white/75 sm:text-lg lg:text-xl">
                ...
                <div class="mt-8 flex flex-wrap items-center justify-center gap-4">
                    @auth
                        <a href="{{ route('dashboard') }}"
                           class="inline-flex items-center gap-2 rounded-full bg-white px-7 py-3.5 text-sm font-bold text-[#8C0375] shadow-lg shadow-black/15 transition-all duration-200 hover:-translate-y-0.5 hover:bg-white/95 hover:shadow-xl">
                            {{ __('Go to Dashboard') }}
                    ...
                    @else
                        ...
                        <a href="{{ route('login') }}"
                           class="inline-flex items-center gap-2 rounded-full border border-white/30 px-7 py-3.5 text-sm font-bold text-white transition-all duration-200 hover:-translate-y-0.5 hover:border-white/50 hover:bg-white/10">
                            {{ __('Sign In') }}
                        </a>
                    @endauth
                </div>
                ...
                {{-- Trust indicators --}}
                <div class="mt-10 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-sm text-white/50">
    ...
    <section class="bg-white py-16 sm:py-20">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
            ...
            <div class="mt-12 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
                @php
                    $instruments = [
                        ['name' => 'Piano',    'icon' => 'piano',    'desc' => 'Classical & contemporary'],
                        ['name' => 'Guitar',   'icon' => 'guitar',   'desc' => 'Acoustic & electric'],
                        ['name' => 'Violin',   'icon' => 'violin',   'desc' => 'Orchestral & solo'],
                        ['name' => 'Voice',    'icon' => 'voice',    'desc' => 'Singing & vocal training'],
                        ['name' => 'Saxophone','icon' => 'saxophone','desc' => 'Jazz & classical'],
                ...
                @foreach ($instruments as $instrument)
                    <div class="group relative overflow-hidden rounded-2xl border border-[color:var(--soh-gray)]/25 bg-[linear-gradient(160deg,#FFFFFF_0%,#FAF5FC_100%)] p-6 transition-all duration-300 hover:-translate-y-1 hover:border-[color:var(--soh-purple)]/40 hover:shadow-[0_12px_36px_rgba(166,18,141,0.14)]">
                        ...
                        <p class="mt-1 text-xs text-gray-500">{{ $instrument['desc'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-3">
                ...
                <div class="rounded-2xl border border-[color:var(--soh-gray)]/25 bg-white p-7 shadow-[0_8px_30px_rgba(166,18,141,0.08)]">
                    ...
                    <p class="mt-2 text-sm leading-relaxed text-gray-600">
                        {{ __('Review assigned lessons, track homework, and update your activity status.') }}
                    </p>
                ...
                <div class="rounded-2xl border border-[color:var(--soh-gray)]/25 bg-white p-7 shadow-[0_8px_30px_rgba(166,18,141,0.08)]">
                    ...
                    <p class="mt-2 text-sm leading-relaxed text-gray-600">
                        {{ __('Manage users, oversee school operations, and keep platform content organized.') }}
                    </p>
                    <a href="{{ route('login') }}" class="soh-link mt-4 inline-flex items-center gap-1 text-sm font-semibold">
                        {{ __('Admin Login') }}
                        <svg class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M7.21 14.77a.75.75 0 01.02-1.06L11.168 10 7.23 6.29a.75.75 0 111.04-1.08l4.5 4.25a.75.75 0 010 1.08l-4.5 4.25a.75.75 0 01-1.06-.02z" clip-rule="evenodd"/></svg>
    ...
    <section class="relative overflow-hidden bg-[linear-gradient(160deg,#A6128D_0%,#6B025E_100%)] py-16 sm:py-20">
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-10 text-center">
            ...
            <div class="mt-8 flex flex-wrap items-center justify-center gap-4">
                @guest
                    <a href="{{ route('register') }}"
                       class="inline-flex items-center gap-2 rounded-full bg-white px-7 py-3.5 text-sm font-bold text-[#8C0375] shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl">
                        {{ __('Enroll Now') }}
                    </a>
                    <a href="{{ url('/contact') }}"
                       class="inline-flex items-center gap-2 rounded-full border border-white/30 px-7 py-3.5 text-sm font-bold text-white transition-all duration-200 hover:-translate-y-0.5 hover:border-white/50 hover:bg-white/10">
