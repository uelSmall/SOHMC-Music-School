                ...
                @else
                    <div class="grid gap-2 sm:grid-cols-2">
                        @foreach($parents as $parent)
                            <label class="flex items-center gap-2 rounded-xl border border-gray-200 px-4 py-2.5 transition hover:border-[#A6128D]/50 hover:bg-[#A6128D]/5">
                                <input type="checkbox" name="parents[]" value="{{ $parent->id }}" {{ in_array($parent->id, old('parents', [])) ? 'checked' : '' }} class="rounded border-gray-300 text-[#A6128D] focus:ring-[#A6128D]/20" />
                                <span class="text-sm">
